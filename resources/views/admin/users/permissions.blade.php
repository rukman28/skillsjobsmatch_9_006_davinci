<x-admin-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Permissions') }} Assigned to <i>{{ $user->name }}</i>
        </h2>
    </x-slot>
                                    ...
                                    <table class="w-full border text-center">
                                        ...
                                        @foreach($permissions as $permission)
                                            <tr class="border-b">
                                                ...
                                                <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap border-r">
                                                    <form method="POST" action="{{ route('admin.users.permissions.revoke', [$user, $permission]) }}" onsubmit="return confirm('Are you sure?');">
                                                        @csrf
                                                        @method('DELETE')
